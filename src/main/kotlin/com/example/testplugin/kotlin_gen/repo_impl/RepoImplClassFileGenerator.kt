...
class RepoImplClassFileGenerator {

    fun generateRepoImplClassFile(
        packageDeclare: String,
        fileName: String,
        apiInterface: ApiInterface,
        parentClass: RepoInterface,
        project: Project?,
        psiFileFactory: PsiFileFactory,
        directory: PsiDirectory,
    ):KotlinClass {
        val fileNamesWithoutSuffix = currentDirExistsFileNamesWithoutKTSuffix(directory)
        var kotlinClassForGenerateFile = RepoImplClassMaker(fileName,apiInterface,parentClass).makeKotlinClass()
